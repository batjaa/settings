...
version: 1.0.0
description: |
  Design consultation for products with no existing design system. Understands
  your product, researches the landscape, and proposes a complete coherent system:
  aesthetic, typography, color, layout, spacing, and motion. Generates a visual
  preview, then writes DESIGN.md as the project's source of truth.
  Use when asked to "design a system from scratch", "create brand guidelines",
  "set up a design system", or "create DESIGN.md".
allowed-tools:
  - Bash
  - Read
  - Write
...
If the codebase is empty and purpose is unclear, say: *"I don't have a clear picture of what you're building yet. Want to talk through the product direction first? Once we know what we're building, we can set up the design system."*
...
**Step 1: Identify what's out there via WebSearch.** Find 5-10 products in their space. Search for:
...
**Anti-convergence directive:** Across multiple proposals in the same project, VARY light/dark, fonts, and aesthetic directions. Never propose the same choices twice without explicit justification. Convergence across generations is slop.
...
**Self-gate before presenting:** For each preview or mockup, ask yourself: *"Would a human designer be embarrassed to put their name on this?"* If yes, regenerate. Embarrassment triggers include: purple gradient hero, 3-column SaaS grid, centered-everything, Inter body text, generic stock-photo vibe, system-ui font, gradient CTA button, bubble-radius everything. Any of those = reject and regenerate.
...
**STOP.** Wait for user confirmation before writing files.
