...
description: |
  ...
  Text actually reflows, heights are computed, layouts are dynamic, cards
  size themselves, chat bubbles shrinkwrap, editorial spreads flow around
  obstacles. Uses the vendored Pretext typographic engine (30KB, zero deps)
  for canvas-based text layout. Smart API routing picks the right Pretext
  pattern for each design type. Use when asked to "finalize this design",
  "turn this into HTML", "build me a page", or "implement this design".
allowed-tools:
  - Bash
  - Read
  - Write
...
Detect what design context exists. Check for:

- A user-provided approved mockup PNG.
- A `DESIGN.md` in the repo root (design tokens / brand colors / fonts).
- A user description of what they want to build.
- Any existing finalized HTML the user wants to evolve.

Then route based on what was found:

### Case A: User has an approved mockup PNG

Read the PNG inline using the Read tool to extract colors, typography, and
component structure. Read `DESIGN.md` if it exists — these tokens take priority
for system-level values (fonts, brand colors, spacing scale).
...
### Case B: No mockup, but the user has a written plan or description

Read whatever context exists (a written plan, design notes, or `DESIGN.md`).

Use AskUserQuestion:
> No approved design mockup found.
> A) I'll design the HTML directly from the plan/description
> B) I have a PNG — let me provide the path

If A: proceed to Step 1 in "plan-driven mode." The plan/description is the source
of truth. Ask the user for a screen name to use for the output file
(e.g., "landing-page", "dashboard", "pricing").
If B: accept a PNG file path from the user and proceed with that as the reference.

### Case C: Nothing — clean slate

Use AskUserQuestion to gather what the user wants to build. Ask about:
purpose/audience, visual feel (dark/light, playful/serious, dense/spacious),
content structure (hero, features, pricing, etc.), and any reference sites
they like. Proceed to Step 1 in "freeform mode." Ask for a screen name.
...
- **Screen name:** user-provided or inferred from context

---

## Step 1: Design Analysis

1. If an approved PNG exists, read it inline using the Read tool. Describe the
   visual layout, colors, typography, and component structure yourself.

2. If in plan-driven or freeform mode (no approved PNG), design from context:
   - **Plan-driven:** read the plan/notes. Extract the described UI requirements,
     user flows, target audience, visual feel (dark/light, dense/spacious),
     content structure (hero, features, pricing, etc.), and design constraints.
     Build an implementation spec from the plan's prose rather than a visual
     reference.
   - **Freeform:** use AskUserQuestion to gather what the user wants to build.
   In both cases, describe the intended visual layout, colors, typography, and
   component structure as your implementation spec. Generate realistic content
   based on the plan or user description (never lorem ipsum).
...
Analyze the design and classify it into a Pretext tier. Each tier uses different
Pretext APIs for optimal results:
...
```bash
_PRETEXT_VENDOR=""
[ -f ~/.claude/skills/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=~/.claude/skills/design-html/vendor/pretext.js
...
Write a single file using the Write tool. Choose an output path that fits the
project (e.g., `./designs/<screen-name>.html` or a path the user specifies).

For framework output, save as `.tsx`, `.svelte`, or `.vue` as appropriate.
...
Pretext handles bidi, CJK, soft-hyphens, and emoji correctly. Always use it for
text layout, even for simple cases — the overhead is 30KB and the correctness
guarantees compound as designs get more complex.
...
echo "SERVER: http://localhost:$_PORT/<filename>"
...
Tell the user: "Live preview running at http://localhost:$_PORT/<filename>.
...
```
LOOP:
  1. If server is running, tell user to open http://localhost:PORT/<filename>
     Otherwise: open <path>/<filename> directly in a browser.
...
> and create a DESIGN.md for your project. This means future design work will be
